...
public class RollController {
    ...
    public String sanCheck(MessageData<?> data) {
        ...
        //优先检测指令是否包含有数值
        if (data.getMessage().matches("^(([1-9]|[1-9][0-9])?[dD]?([1-9][0-9][0-9]|[1-9][0-9]|[1-9])\\+{0,1}){1,10}/(([1-9]|[1-9][0-9])?[dD]?([1-9][0-9][0-9]|[1-9][0-9]|[1-9])\\+?){1,10} ([0-9]|[1-9][0-9]|100)$")) {
        ...
        //检测用户输入的指令格式是否正确
        if (data.getMessage().matches("^(([1-9]|[1-9][0-9])?[dD]?([1-9][0-9][0-9]|[1-9][0-9]|[1-9])\\+{0,1}){1,10}/(([1-9]|[1-9][0-9])?[dD]?([1-9][0-9][0-9]|[1-9][0-9]|[1-9])\\+?){1,10}")) {
            //查询用户数据
            String attribute = userTempDataService.getUserAttribute(data.getQqID());
